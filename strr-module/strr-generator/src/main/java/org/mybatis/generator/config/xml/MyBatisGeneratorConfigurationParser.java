...
import org.mybatis.generator.config.JavaCrudGeneratorConfiguration;
...
public class MyBatisGeneratorConfigurationParser {
    ...
    private void parseContext(Configuration configuration, Node node) {
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            // 自定义 generator 配置解析
            } else if ("javaCrudGenerator".equals(childNode.getNodeName())) { //$NON-NLS-1$
                parseJavaCrudGenerator(context, childNode);
     ...
     * @param node
     */
    private void parseJavaCrudGenerator(Context context, Node node) {
        Properties attributes = parseAttributes(node);
        String targetPackage = attributes.getProperty("targetPackage"); //$NON-NLS-1$
        String targetProject = attributes.getProperty("targetProject"); //$NON-NLS-1$
        String clientProject = attributes.getProperty("clientProject");
        // 自定义配置
        JavaCrudGeneratorConfiguration javaCrudGeneratorConfiguration = new JavaCrudGeneratorConfiguration();
        context.setJavaCrudGeneratorConfiguration(javaCrudGeneratorConfiguration);
        javaCrudGeneratorConfiguration.setTargetPackage(targetPackage);
        javaCrudGeneratorConfiguration.setTargetProject(targetProject);
        javaCrudGeneratorConfiguration.setClientProject(clientProject);
        // java model
        JavaModelGeneratorConfiguration javaModelGeneratorConfiguration = new JavaModelGeneratorConfiguration();
